...
func (a *App) buildBotHandoff(apiKey string) string {
	...
	builder.WriteString(apiKey)
	builder.WriteString("\\n\\n")
	builder.WriteString("Approval policy:\\n")
	builder.WriteString("- Read-only API calls do not need manual approval.\\n")
	builder.WriteString("- Mutating API calls can run without approval, except the first time a post is scheduled when accept_before_planning=true.\\n")
	builder.WriteString("- Once a post has been approved for planning, later edits/reschedules for that same post do not need another approval.\\n\\n")
	...
	builder.WriteString("4) GET /api/v1/webhooks/replays for replay queue\\n")
	builder.WriteString("5) POST /api/v1/webhooks/replays/{id}/replay for manual replay\\n")
	builder.WriteString("6) POST /api/v1/posts for new scheduled posts\\n")
	builder.WriteString("7) POST /api/v1/posts/{id}/send-now for immediate send\\n\\n")
